...
static_files_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),'static/dlib/')
...
def facial_landmarks(image, eyeOnlyMode=False, allowEnhancement=False):
    ...
    predictor68 = dlib.shape_predictor(static_files_path+'shape_predictor_68_face_landmarks.dat')
    predictor81 = dlib.shape_predictor(static_files_path+'shape_predictor_81_face_landmarks.dat')
...
def drawPoints(image, points, pointColor=(249,249,249), lineColor=(255,0,0), pointThickness=None, lineThickness=1):
...
def lineFeatures(image, graphical_pts, thickness=None):
    ...
    colors = {
            'forehead height': (255,0,255),
            'middle face height': (205,0,0),
            'lower face height': (127,255,0),
            'left eye area': (255,144,30),
            'right eye area': (170,232,238),
            'eye to eye distance': (119,115,13),
            'eye to eyebrow distance': (47,255,173),
            'upper lip height': (0,0,255),
            'lower lip height': (173,222,255),
            'eyebrows distance': (255,248,240),
            'nose length': (0,140,255),
            'nose width': (0,255,255),
            'nose arc': (114,128,250),
            'eyebrow shape detector 1': (255,191,0),
            'eyebrow shape detector 2': (147,20,255),
            'eye slope detector1': (32,96,255),
            'eye slope detector2': (255,255,0),
            'eyebrow slope': (160,255,224)
            }
