...
def drawPoints(image, points, pointColor=(249,249,249), lineColor=(255,255,255), pointThickness=None, lineThickness=1):
    # Function to draw points on facial features
    if pointThickness is None:
        pointThickness = round((7/1200) * image.shape[1])
        print("THICKNESS")
        print(pointThickness)
    for i in points:
        x,y = i
...
def detect(request):
    if request.method == 'POST':
        ...
        if eyePoints is not None:
            ...
            return JsonResponse({'image': output_image_html})
        else:
